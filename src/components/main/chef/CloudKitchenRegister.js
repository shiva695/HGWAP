...
const CloudKitchenRegister = () => {
  ...
  const [cloudKitchenNameError, setCloudKitchenNameError] = useState(false);
  const [cloudKitchenNameHelperText, setCloudKitchenNameHelperText] =
    useState("");
      ...
      case 0:
        return (
          <Box sx={{ width: { xs: "90%", md: "750px" } }}>
            ...
            <TextField
              id="cloudKitchenName"
              label="Restaurant Name (Brand Name) *"
              value={cloudKitchenName}
              variant="standard"
              onChange={(ev) => {
                setCloudKitchenName(ev.target.value.replace(/\s\s+/g, " "));
                setCloudKitchenNameError(false);
                setCloudKitchenNameHelperText("");
              }}
              onKeyUp={() => verifyHomeChefName()}
              inputProps={{ maxLength: 40 }}
              error={cloudKitchenNameError}
              helperText={cloudKitchenNameHelperText}
              sx={{ mb: 2 }}
              fullWidth
            />
      ...
      case 4:
        return (
          ...
          >
            ...
            <Typography variant="bodybold">{cloudKitchenName}</Typography>
            <Typography variant="bodybold">{outletName}</Typography>
  ...
  const verifyHomeChefName = async () => {
    if (cloudKitchenName.trim().length >= 3) {
      let params = {
        chefName: cloudKitchenName.trim(),
      };
      ...
      if (response.status >= 200 && response.status < 300) {
        if (
          ...
          setCloudKitchenNameError(true);
          setCloudKitchenNameHelperText(
            "This name is already taken. Please try with a different name."
          );
        } else {
          alert(
            "Something went wrong while checking Restaurant name. Please try again later!"
          );
        }
      } else {
        alert(
  ...
  const validateStep0 = () => {
    ...
    if (!previewFile) {
      setPreviewFileError("Please upload your Logo");
      validationErrors = true;
    }

    if (cloudKitchenName.trim().length < 3) {
      setCloudKitchenNameError(true);
      setCloudKitchenNameHelperText("Please enter a valid name");
      validationErrors = true;
    }

    if (cloudKitchenNameError) {
      validationErrors = true;
    }
  ...
  // On load
  useEffect(() => {
    document.title = config.documentTitle + " | Register as Restaurant";
  }, []);
  ...
  useEffect(() => {
    const invokeChefRegistrationApi = async () => {
      ...
      let params = {
        cloudKitchenName: cloudKitchenName.trim(),
      ...
      let response = await invokeApi(
        config.apiDomains.chefService + apiList.registerCloudKitchen,
        params,
        cookies
      );
      if (response.status >= 200 && response.status < 300) {
              ...
              <DialogContent>
                ...
                >
                  ...
                  >
                    ...
                    Restaurant registration form has been submitted
                      successfully.
                    </Typography>
                    <Typography variant="bodyparagraph">
...
export default CloudKitchenRegister;
