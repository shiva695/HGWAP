...
import { getChef, getUser } from "../../../global/redux/actions";
...
const ProfileChefView = () => {
  ...
  const {
    isFetching: isChefDataFetching,
    chefData,
    chefError,
    logout: chefReducerLogout,
  } = globalState.chefReducer;
  ...
  const [isHomechefConfigFetching, setIsHomechefConfigFetching] =
    useState(false);
  ...
  const maxImgsLen = 15;
  let approvedImageFilter = chefImages?.filter(
    (el) => el.imageType === "Secondary" && el.status === "Active"
  );
  let pendingImageFilter = chefImages?.filter(
    (el) => el.imageType === "Secondary" && el.status === "Pending"
  );
  ...
  // Update chef profile address;
  const updateChefProfileAddress = async () => {
    let validateProfile = validateChefData();
    if (validateProfile) {
      ...
      let response = await invokeApi(
        config.apiDomains.chefService + apiList.updateHomeChefAddress,
        params,
        cookies
      );
      if (response.status >= 200 && response.status < 300) {
        if (response.data.responseCode === "200") {
          ...
          alert(
            "Something went wrong while updating home chef address. Please try again later!"
          );
          setIsUpdateChefProfileFetching(false);
        }
      } else if (response.status === 401) {
  ...
  const handleFileUpload = (ev) => {
    ...
    if (fileUploaded && acceptProfileFileTypes) {
      if (fileUploaded.size < 5242880) {
        ...
        setMaxFileSizeErr("");
        chefProfileUpdateStatus(fileUploaded);
      } else {
  ...
  const chefProfileUpdateStatus = async (file) => {
    // Primary/profile pic change
    ...
    if (response.status >= 200 && response.status < 300) {
      if (response.data.responseCode === "200") {
        let primImg = chefImages.filter((arr) => arr.imageType === "Primary");

        let profileImgs = galleryImageURLs;
        profileImgs.push({
          galleryImage: response.data.imageUrl,
          id: primImg[0].id,
        });
        setGalleryImageURLs(profileImgs);
  ...
  // Home chef config
  const updateHomeChefConfig = async () => {
    setIsHomechefConfigFetching(true);
    let params = {
      id: chefData?.homeChef.id,
    ...
    let response = await invokeApi(
      config.apiDomains.chefService + apiList.updateHomeChefConfig,
      params,
      cookies
    );
    if (response.status >= 200 && response.status < 300) {
      if (response.data.responseCode === "200") {
        ...
        setIsHomechefConfigFetching(false);
      } else {
        alert(
  ...
  // On load
  useEffect(() => {
    document.title = config.documentTitle + " | Home Chef Profile";
  }, []);
      ...
      if (userData?.user.roles.indexOf("Home Chef Owner") === -1) {
        ...
        if (!chefData) {
          ...
          setPreferenceChecked(
            chefData?.homeChef.whatsappPreference === "Yes" ? true : false
          );
          setIsOrderAccepted(
            chefData?.homeChef.deliveryAssignmentAction === "orderAccepted"
              ? "Yes"
              : "No"
          );
          setCoPlatformFee(chefData?.homeChef.coPlatformFee);
        }
      }
    }
  }, [userData, dispatch, cookies, chefData, navigate]);
  ...
  useEffect(() => {
    const getChefManagers = async () => {
      ...
      if (response.status >= 200 && response.status < 300) {
        if (response.data.responseCode === "200") {
          let copy = JSON.parse(JSON.stringify(phoneNumberData));

          copy.push({
            userId: chefData?.homeChef.userId,
            mobileNumber: `${chefData?.homeChef.userFullName ?? ""} - ${
              chefData?.homeChef.userMobileNumber
            }`,
          });
          ...
          if (filterActiveManagers.length > 0) {
            filterActiveManagers?.map((el) =>
              copy.push({
                ...
                mobileNumber: `${el.managerUserName ?? ""} - ${
          ...
          let defaultNo = copy.filter(
            (el) => el.userId === chefData?.homeChef?.deliveryCallsUserId
          );
    ...
    if (invokeGetChefManagers && chefData?.homeChef && !isChefDataFetching) {
  ...
  useEffect(() => {
    if (logout || chefReducerLogout) {
      navigate("/logout");
    }
  }, [logout, chefReducerLogout, navigate]);

  // setting initial state of chefData
  useEffect(() => {
    if (chefData?.homeChef) {
      let primaryImage = chefData?.homeChef.galleryImages.filter(
        (arr) => arr.imageType === "Primary"
      );
      setPreviewFile(primaryImage[0]?.galleryImage);

      setMarkerPositionView({
        lat: parseFloat(chefData?.homeChef.latitude),
        lng: parseFloat(chefData?.homeChef.longitude),
      });
      ...
      // GST data for drawer
      let chefDetails = chefData?.homeChef;
      setGstNumber(chefDetails.gstNumber);
      setIsGstRegistered(chefDetails.isGstRegistered);
      setGstCertificateFile(chefDetails.gstCertificate);

      // Chef gallery images
      const allGalleryImgs = chefData?.homeChef.galleryImages.map((item) => ({
        id: item.id,
        status: item.status,
  ...
  useEffect(() => {
    const updateHomechefGst = async () => {
      setIsUpdateGstFetching(true);
      let chefId = chefData?.homeChef.id;
      let params = {
        id: chefId,
        isGstRegistered,
        gstNumber,
      ...
      let response = await invokeApi(
        config.apiDomains.chefService + apiList.updateHomeChefGST,
        params,
        cookies
      );
      if (response.status >= 200 && response.status < 300) {
  ...
  useEffect(() => {
    if (fssaiId) {
      let chefDetails = chefData?.homeChef;
      let fssaiArray = chefDetails.fssaiLicences.filter(
  ...
  useEffect(() => {
    const updateHomechefFssai = async () => {
      ...
      let response = await invokeApi(
        config.apiDomains.chefService + apiList.updateHomeChefFSSAI,
        params,
        cookies
      );
      if (response.status >= 200 && response.status < 300) {
  ...
  useEffect(() => {
    const updateImages = async () => {
      ...
      let response = await invokeApi(
        config.apiDomains.chefService + apiList.updateHomeChefGalleryImages,
        params,
        cookies
      );
      if (response.status >= 200 && response.status < 300) {
    ...
    if (profileUploadStatus || secondaryImageStatus) {
      setSecondaryImageStatus(false);
      setProfileUploadStatus(false);
      updateImages();
    }
      ...
      {isLoading || isChefDataFetching ? (
        ...
        >
          ...
          >
            {(previewFile || uploadedFile) && !maxFileSizeErr ? (
              ...
              >
                ...
                <Box
                  ...
                  src="/media/svg/home-chef-ribbon.svg"
                />
              </Box>
            ) : (
            ...
            {chefData.homeChef.galleryImages.map(
              (item) =>
            ...
            {chefData?.homeChef.physicalVerificationStatus === "Rejected" && (
              <>
                <Typography variant="bodyparagraph">
                  Physical verification status:{" "}
                  {chefData?.homeChef.physicalVerificationStatus}
                </Typography>
                <Typography variant="bodyparagraph">
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <Typography variant="bodyparagraph">
                      {chefData.homeChef.streetAddress},{" "}
                      {chefData.homeChef.locality}, {chefData.homeChef.city},{" "}
                      {chefData.homeChef.state}, {chefData.homeChef.pincode}
                    </Typography>
                  </Box>
                  <Typography variant="bodyparagraph">
                    Profile Status: {chefData.homeChef.profileApprovalStatus}
                  </Typography>
                  {chefData.homeChef.profileApprovalStatus === "Rejected" && (
                    <Typography variant="bodyparagraph">
                      Rejection Remarks:{" "}
            ...
            >
              ...
              {chefData?.homeChef.gstNumber ? (
                ...
                >
                  ...
                  <Typography variant="bodyparagraph">
                    GST Status: {chefData?.homeChef.gstApprovalStatus}
                  </Typography>
                  <Tooltip title="Download">
                    <IconButton
                      onClick={() => {
                        window.open(
                          chefData?.homeChef.gstCertificate,
                          "_blank"
                        );
                      }}
                    >
              ...
              {chefData.homeChef.gstApprovalStatus === "Rejected" && (
                <Typography variant="bodyparagraph">
                  Rejection Remarks: {chefData.homeChef.gstApprovalRemarks}
                </Typography>
              )}
            </Box>
          )}
                ...
                {chefData?.homeChef.fssaiLicences.length > 0 ? (
                  <TableContainer component={Paper}>
                    ...
                    >
                      ...
                      <TableBody>
                        {chefData?.homeChef.fssaiLicences?.map((row, idx) => (
                          <TableRow
                            key={idx}
                            sx={{
          ...
          {tabIndex === 3 && (
            ...
            >
              ...
              {chefImages?.some(
                (arr) =>
                  (arr.status === "Pending" || arr.status === "Active") &&
                  arr.imageType === "Secondary"
              ) && (
                <FormControlLabel
                  sx={{ float: "right" }}
                  control={
            ...
            >
              ...
              <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
                <Button
                  ...
                  onClick={() => {
                    updateHomeChefConfig();
                    setSubmitStatus(false);
                  }}
                  variant="contained"
                >
                  {isHomechefConfigFetching ? (
                    <CircularProgress size={24} sx={{ ml: 2 }} />
                  ) : (
                    <></>
                  )}
        ...
        >
          ...
          >
            ...
            >
              ...
              <Button
                onClick={updateChefProfileAddress}
                type="submit"
            ...
            >
              <Button
                onClick={() => {
                  ...
                  setGstCertHelperText("");
                  setGstUploadFile(null);
                  setGstNumber(chefData.homeChef.gstNumber);
                }}
                variant="outlined"
                size="small"
                color="primary"
...
export default ProfileChefView;
