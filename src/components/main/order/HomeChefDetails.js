...
const HomeChefDetails = () => {
  ...
  const { id: homeChefId, name: chefName } = useParams();
  const itemDescRefs = useRef([]);

  const globalState = useSelector((state) => state);
  const { userData } = globalState.userReducer;

  const [homeChefData, setHomeChefData] = useState(null);
  ...
  const cloudKitchenOutletId = null;
  ...
  const handleFavourite = async (id, favorite) => {
    if (favorite === "Active") {
      ...
      if (response.status >= 200 && response.status < 300) {
        if (response.data.responseCode === "200") {
          let copy = JSON.parse(JSON.stringify(homeChefData));
          copy.isFavorite = "InActive";
          setHomeChefData(copy);
  ...
  // Get home chef data first time and when any filters are applied (which in turn update cookies)
  useEffect(() => {
    const getHomeChef = async () => {
      let params = {
        id: homeChefId,
      ...
      if (response.status >= 200 && response.status < 300) {
        if (response.data.responseCode === "200") {
          if (response.data.homeChef === null) {
            navigate("404");
          }
          if (
            chefName !== response.data.homeChef.chefName?.replace(/\s+/g, "-")
          ) {
            window.history.replaceState(
              null,
              "",
              `/chef/${response.data.homeChef.chefName?.replace(
                /\s+/g,
                "-"
              )}/${homeChefId}`
            );
          ...
          document.title =
            config.documentTitle + " | " + response.data.homeChef.chefName;
          if (response.data.homeChef.foodItems === null) {
            let copy = JSON.parse(JSON.stringify(response.data.homeChef));
            copy.foodItems = [];
            setHomeChefData(copy);
            setGroupedFoodItems([]);
          } else {
            setHomeChefData(response.data.homeChef);
            setGroupedFoodItems(
              response.data.homeChef.foodItems?.reduce((group, product) => {
          ...
          setTotalFoodItem(response.data.homeChef?.foodItems?.length ?? 0);
        } else {
          alert(
    ...
    getHomeChef();
  }, [chefName, cookies, homeChefId, navigate]);
  ...
  useEffect(() => {
    let dist = getDistanceFromLatLonInKm(
      ...
      homeChefData?.latitude,
      homeChefData?.longitude
    );
    if (!!dist) {
      setDistance(dist);
    }
  }, [homeChefData, cookies]);
  ...
  }, [cookies, homeChefId, userData, deliveryType, homeChefData]);
  ...
  useEffect(() => {
    ...
    if (invokeCheckAddress && addressData !== null && !!homeChefData) {
  ...
  }, [userData, cartData, orderType, preOrderDate, deliverySlot, homeChefId]);
  ...
  return (
    <Box>
      ...
      {!homeChefData ? (
        ...
        <>
          {/* Gallery images */}
          {homeChefData?.galleryImages && (
            <Carousel animation="slide" duration={800} sx={{ mt: "29px" }}>
              {homeChefData?.galleryImages?.map((el, idx) => (
                <Box
                  key={idx}
                  component="img"
              ...
              >
                ...
                >
                  ...
                  <Box
                    ...
                    src={
                      homeChefData.isFavorite === "Active"
                  ...
                  {homeChefData?.noOfRatings >= 10 && (
                    ...
                    >
                      ...
                      <Typography
                        ...
                        sx={{ color: "#4D4D4D" }}
                      >
                        {homeChefData.averageRating.toFixed(1)} (
                        {ratingsGroup(homeChefData.noOfRatings)})
                      </Typography>
                    </Box>
                  )}
                ...
                >
                  {homeChefData?.cuisines?.map((cuis, idx2) => (
                    <Box
                      key={idx2}
                      sx={{
            ...
            >
              ...
              >
                ...
                <Typography variant="bodyparagraph">
                  {homeChefData?.locality}, {homeChefData?.city}
                </Typography>
              </Box>
            ...
            <Box sx={{ width: "100%" }}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={9} md={6} lg={7}>
                  ...
                  >
                    {homeChefData?.foodItems?.length > 0 ? (
                      <>
                        {Object.values(groupedFoodItems)?.map((el, idx) => (
                          <Box
                            key={idx}
                          ...
                          >
                            <Typography variant="header4" sx={{ mt: "8px" }}>
                              {el[0].category}
                            </Typography>
                            {/* Food items list */}
                            {el.map((item, indx) => (
                              <Box
                                // onClick={() => {
                                //   navigate(
                                //     `/food/${homeChefData?.chefName.replace(
                                //       /\s+/g,
                                //       "-"
          ...
          {!!homeChefData?.fssaiLicenseNo && (
            ...
            >
              ...
              <Typography variant="bodybold">
                License No: {homeChefData?.fssaiLicenseNo}
              </Typography>
            </Box>
          )}
        </>
